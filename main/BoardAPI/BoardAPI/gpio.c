...
bool gpio_getUsbId() {
	return (READ_PIN(USB_ID) == GPIO_PIN_RESET);
}

bool gpio_getUsrBtn() {
	return (READ_PIN(USR_BTN) == GPIO_PIN_RESET);
}

void gpio_setLedR(bool state) {
	WRITE_PIN(LED_R, state ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

void gpio_setLedG(bool state) {
	WRITE_PIN(LED_G, state ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

void gpio_setLedB(bool state) {
	WRITE_PIN(LED_B, state ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
