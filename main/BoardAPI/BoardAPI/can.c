...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	if(hcan == &hcan1) {
		...
		bool isExtended = (can1_rxHeader.IDE == CAN_ID_EXT);
		bool isRemote = (can1_rxHeader.RTR == CAN_RTR_REMOTE);
		uint32_t id = isExtended ? can1_rxHeader.ExtId : can1_rxHeader.StdId;
		uint8_t *data = can1_buffer;
		uint8_t dlc = can1_rxHeader.DLC;
		if(can1_receivedCallback != NULL) can1_receivedCallback(id, data, dlc, isExtended, isRemote);
	}else if(hcan == &hcan2) {
		HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &can2_rxHeader, can2_buffer);
		bool isExtended = can2_rxHeader.IDE == CAN_ID_EXT;
		bool isRemote = can2_rxHeader.RTR == CAN_RTR_REMOTE;
		uint32_t id = isExtended ? can2_rxHeader.ExtId : can2_rxHeader.StdId;
		uint8_t *data = can2_buffer;
		uint8_t dlc = can2_rxHeader.DLC;
		if(can2_receivedCallback != NULL) can2_receivedCallback(id, data, dlc, isExtended, isRemote);
	}
}
