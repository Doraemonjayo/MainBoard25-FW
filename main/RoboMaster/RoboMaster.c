...
const PIDConfig ROBOMASTER_M2006_PID_VELOCITY_DEFAULT = { 0.05f, 0.5f, 0.0f,
		0.001f, -INFINITY,
		INFINITY, -ROBOMASTER_M2006_MAX_CURRENT,
		ROBOMASTER_M2006_MAX_CURRENT, -ROBOMASTER_M2006_MAX_CURRENT,
		ROBOMASTER_M2006_MAX_CURRENT, -INFINITY,
		INFINITY, 0.0f, 0.0f, };

const PIDConfig ROBOMASTER_M2006_PID_POSITION_DEFAULT = { 50.0f, 0.0f, 2.0f,
		0.001f, -INFINITY,
		INFINITY, -INFINITY,
		INFINITY, -INFINITY,
		INFINITY, -INFINITY,
...
const PIDConfig ROBOMASTER_M3508_PID_VELOCITY_DEFAULT = { 0.2f, 5.0f, 0.0f,
		0.001f, -INFINITY,
		INFINITY, -ROBOMASTER_M3508_MAX_CURRENT,
		ROBOMASTER_M3508_MAX_CURRENT, -ROBOMASTER_M3508_MAX_CURRENT,
		ROBOMASTER_M3508_MAX_CURRENT, -INFINITY,
		INFINITY, 0.0f, 0.0f, };

const PIDConfig ROBOMASTER_M3508_PID_POSITION_DEFAULT = { 50.0f, 0.5f, 0.0f,
		0.001f, -INFINITY,
		INFINITY, -INFINITY,
		INFINITY, -INFINITY,
		INFINITY, -INFINITY,
...
void RoboMaster_resetFeedback(RoboMaster *robomaster) {
	...
	robomaster->feedback.temperature = -1;
...
void RoboMaster_init(RoboMaster *robomaster, const RoboMasterConfig *config) {
	robomaster->config = *config;
	RoboMaster_reset(robomaster);
}

void RoboMasters_setTxFunc(RoboMasters *robomasters,
		void (*canTxFunc)(uint32_t id, const uint8_t *data, uint8_t dlc,
		bool isExtended, bool isRemote)) {
...
void RoboMaster_setTimeout(RoboMaster *robomaster, uint32_t controlTimeout,
		uint32_t feedbackTimeout) {
	RoboMaster_setControlTimeout(robomaster, controlTimeout);
	RoboMaster_setFeedbackTimeout(robomaster, feedbackTimeout);
}

void RoboMaster_setCurrentLimit(RoboMaster *robomaster, float minCurrent,
		float maxCurrent) {
	robomaster->config.pidVelocity.config.minOut = minCurrent;
	robomaster->config.pidVelocity.config.maxOut = maxCurrent;
}

void RoboMaster_setVelocityLimit(RoboMaster *robomaster, float minVelocity,
		float maxVelocity) {
...
void RoboMaster_setPositionLimit(RoboMaster *robomaster, float minPosition,
		float maxPosition) {
	robomaster->config.pidPosition.config.minTarget = minPosition;
	robomaster->config.pidPosition.config.maxTarget = maxPosition;
}

void RoboMaster_setTrapezoidAcceleration(RoboMaster *robomaster,
		float acceleration) {
	robomaster->config.trapezoidAcceleration = acceleration;
}

void RoboMaster_setTrapezoidMaxAcceleration(RoboMaster *robomaster,
		float acceleration) {
...
void RoboMaster_setRawPositionOffset(RoboMaster *robomaster, int16_t offset) {
	...
	robomaster->feedback.position = (robomaster->feedback.roundCount
			* ROBOMASTER_ENCODER_RESOLUTION + robomaster->feedback.rawPosition
			- robomaster->config.rawPositionOffset)
			* (2.0f * PI / ROBOMASTER_ENCODER_RESOLUTION);
}

void RoboMaster_resetZeroPosition(RoboMaster *robomaster) {
	RoboMaster_setRoundCount(robomaster, 0);
	RoboMaster_setRawPositionOffset(robomaster,
			robomaster->feedback.rawPosition);
}
...
void RoboMaster_setTargetCurrent(RoboMaster *robomaster, float current) {
	...
	robomaster->feedback.lastControlTime = ((robomaster->config.getMsFunc != NULL) ? robomaster->config.getMsFunc() : 0);
}
...
void RoboMaster_calculateOutputCurrent(RoboMaster *robomaster) {
	if (((robomaster->config.getMsFunc != NULL) ? robomaster->config.getMsFunc() : 0)
			> robomaster->feedback.lastControlTime
					+ robomaster->config.controlTimeout) {
		RoboMaster_stop(robomaster);
		return;
	}
	if (((robomaster->config.getMsFunc != NULL) ? robomaster->config.getMsFunc() : 0)
			> robomaster->feedback.lastFeedbackTime
					+ robomaster->config.feedbackTimeout) {
		RoboMaster_reset(robomaster);
		return;
	}
	...
	case ROBOMASTER_MODE_CURRENT:
		...
		break;
	case ROBOMASTER_MODE_VELOCITY:
		outputCurrent = PID_calc(&robomaster->config.pidVelocity,
				robomaster->target, robomaster->feedback.velocity);
		break;
	case ROBOMASTER_MODE_POSITION:
		targetVelocity = PID_calc(&robomaster->config.pidPosition,
				robomaster->target, robomaster->feedback.position);
		outputCurrent = PID_calc(&robomaster->config.pidVelocity,
				targetVelocity, robomaster->feedback.velocity);
		break;
	case ROBOMASTER_MODE_TRAPEZOID:
		...
		if (robomaster->config.trapezoidAcceleration
				> robomaster->config.trapezoidMaxAcceleration)
			robomaster->config.trapezoidAcceleration =
					robomaster->config.trapezoidMaxAcceleration;
		...
		if (error >= 0.0f) {
			...
			targetVelocity =
					MIN(
							robomaster->config.pidVelocity.target
									+ robomaster->config.trapezoidAcceleration
											* robomaster->config.pidVelocity.config.dt,
							MAX(robomaster->config.pidVelocity.target - robomaster->config.trapezoidMaxAcceleration * robomaster->config.pidVelocity.config.dt, sqrtf(2.0f * robomaster->config.trapezoidAcceleration * error)));
		} else {
			...
			targetVelocity =
					MAX(
							robomaster->config.pidVelocity.target
									- robomaster->config.trapezoidAcceleration
											* robomaster->config.pidVelocity.config.dt,
							MIN(robomaster->config.pidVelocity.target + robomaster->config.trapezoidMaxAcceleration * robomaster->config.pidVelocity.config.dt, -sqrtf(-2.0f * robomaster->config.trapezoidAcceleration * error)));
		}
		outputCurrent = PID_calc(&robomaster->config.pidVelocity,
				targetVelocity, robomaster->feedback.velocity);
		break;
...
void RoboMaster_transmit(RoboMasters *robomasters) {
	void (*canTxFunc)(uint32_t id, const uint8_t *data, uint8_t dlc,
	bool isExtended, bool isRemote) = robomasters->canTxFunc;
	if (canTxFunc == NULL)
		return;
	uint8_t txData[8];
	int16_t rawCurrents[ROBOMASTER_MOTOR_NUM];
	for (uint8_t i = 0; i < ROBOMASTER_MOTOR_NUM; i++) {
		RoboMaster *robomaster = &robomasters->robomaster[i];
		switch (robomaster->config.model) {
		case ROBOMASTER_M2006:
			rawCurrents[i] = (int16_t) (robomaster->outputCurrent
					* (ROBOMASTER_M2006_MAX_RAW_CURRENT
							/ ROBOMASTER_M2006_MAX_CURRENT));
			rawCurrents[i] = MIN(
					MAX(rawCurrents[i], -ROBOMASTER_M2006_MAX_RAW_CURRENT),
					ROBOMASTER_M2006_MAX_RAW_CURRENT);
			break;
		case ROBOMASTER_M3508:
			rawCurrents[i] = (int16_t) (robomaster->outputCurrent
					* (ROBOMASTER_M3508_MAX_RAW_CURRENT
							/ ROBOMASTER_M3508_MAX_CURRENT));
			rawCurrents[i] = MIN(
					MAX(rawCurrents[i], -ROBOMASTER_M3508_MAX_RAW_CURRENT),
					ROBOMASTER_M3508_MAX_RAW_CURRENT);
			break;
		default:
			rawCurrents[i] = 0;
	...
	canTxFunc(0x200, txData, sizeof(txData), false, false);

	for (uint8_t i = 0; i < 4; i++) {
		txData[i * 2] = (uint8_t) ((rawCurrents[i + 4] >> 8) & 0xFF);
		txData[i * 2 + 1] = (uint8_t) (rawCurrents[i + 4] & 0xFF);
	}
	canTxFunc(0x1FF, txData, sizeof(txData), false, false);
}

void RoboMaster_transmitAllFree(RoboMasters *robomasters) {
	void (*canTxFunc)(uint32_t id, const uint8_t *data, uint8_t dlc,
	bool isExtended, bool isRemote) = robomasters->canTxFunc;
	if (canTxFunc == NULL)
		return;
...
bool RoboMaster_rxTask(RoboMasters *robomasters, uint32_t id, uint8_t *data,
		uint8_t dlc, bool isExtended, bool isRemote) {
	if (isExtended || isRemote || dlc != 8)
		return false;
	...
	robomaster->feedback.lastFeedbackTime = ((robomaster->config.getMsFunc != NULL) ? robomaster->config.getMsFunc() : 0);
	if (robomaster->feedback.rawPosition - robomaster->feedback.lastRawPosition
			> ROBOMASTER_ENCODER_RESOLUTION / 2) {
		robomaster->feedback.roundCount--;
	} else if (robomaster->feedback.rawPosition
			- robomaster->feedback.lastRawPosition
			< -ROBOMASTER_ENCODER_RESOLUTION / 2) {
		robomaster->feedback.roundCount++;
	}
	robomaster->feedback.position = (robomaster->feedback.roundCount
			* ROBOMASTER_ENCODER_RESOLUTION + robomaster->feedback.rawPosition
			- robomaster->config.rawPositionOffset)
			* (2.0f * PI / ROBOMASTER_ENCODER_RESOLUTION);
	robomaster->feedback.velocity = robomaster->feedback.rawVelocity
			* (2.0f * PI / 60.0f);
	robomaster->feedback.temperature = data[6];
	switch (robomaster->config.model) {
	case ROBOMASTER_M2006:
		robomaster->feedback.current = robomaster->feedback.rawCurrent
				* (ROBOMASTER_M2006_MAX_CURRENT
						/ ROBOMASTER_M2006_MAX_RAW_CURRENT);
		break;
	case ROBOMASTER_M3508:
